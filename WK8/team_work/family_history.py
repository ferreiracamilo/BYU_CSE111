...
def print_death_age(people_dict):
    ...
    """
    persons_name_age = []
    for person_line in people_dict:
        person_vals = people_dict[person_line]
        birth_year = person_vals[2]
        death_year = person_vals[3]
        age = death_year - birth_year
        name = person_vals[0]
        persons_name_age.append(f"{name} {age}") #name + " " +age
    
    for person in persons_name_age:
        print(person)
...
def count_genders(people_dict):
    ...
    for person_line in people_dict:
        ...
        if sex == "M":
            ...
            female_qty += 1
    persons_genders.append(f"Number of males: {male_qty}")
    persons_genders.append(f"Number of females: {female_qty}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage in marriages_dict:
        vals = marriages_dict[marriage]
        husband_key = vals[0]
        wife_key = vals[1]
        wedding_year = vals[2]
        if husband_key in people_dict and wife_key in people_dict:
            husband_line = people_dict[husband_key]
            wife_line = people_dict[wife_key]
            marriage_line = f"{husband_line[0]} {wedding_year-husband_line[2]} > {wedding_year} < {wife_line[0]} {wedding_year - wife_line[2]}"
            marriages_formatted.append(marriage_line)
    for marriage_x in marriages_formatted:
        print(marriage_x)
